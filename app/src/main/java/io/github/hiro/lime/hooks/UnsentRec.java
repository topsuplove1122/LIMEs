...
public class UnsentRec implements IHook {
    ...
    public void hook(LimeModule module, ClassLoader classLoader, LimeOptions limeOptions) throws Throwable {
        ...
        try {
                    ...
                    if (dbFile1.exists()) {
                        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O_MR1) {
                            ...
                            try {
                                ...
                                module.log(XposedInterface.LOG_LEVEL_ERROR, "LIMEs", "UnsentRec 開啟資料庫失敗: " + e.getMessage());
                            }
                        }
                    }
                    return result;
            ...
            module.log(XposedInterface.LOG_LEVEL_ERROR, "LIMEs", "UnsentRec (onCreate) Hook 失敗: " + e.getMessage());
        }
    }
    ...
    private void hookMessageDeletion(LimeModule module, ClassLoader classLoader, SQLiteDatabase db1) {
        try {
            ...
            module.log(XposedInterface.LOG_LEVEL_ERROR, "LIMEs", "UnsentRec 找不到 Response Class");
        }
    }
    ...
    private void processMessage(LimeModule module, String paramValue, SQLiteDatabase db1) {
        try {
            ...
            module.log(XposedInterface.LOG_LEVEL_ERROR, "LIMEs", "UnsentRec 處理訊息失敗: " + e.getMessage());
        }
    }
            ...
            if (cursor.moveToFirst()) {
                ...
                try (Cursor existingCursor = db1.rawQuery("SELECT * FROM chat_history WHERE server_id=? AND content=?", new String[]{serverId, canceledContent})) {
                    if (!existingCursor.moveToFirst()) {
                        ...
                        module.log(XposedInterface.LOG_LEVEL_INFO, "LIMEs", "UnsentRec: 成功寫入防收回紀錄！");
                    }
                }
            }
        } catch (Exception e) {
            module.log(XposedInterface.LOG_LEVEL_ERROR, "LIMEs", "UnsentRec 寫入資料庫失敗: " + e.getMessage());
        }
    }
}
